...
public class FastZombieAI : MonoBehaviour, IDamageReceiver
{
    public GameObject target; // Player
    public float speed = 0.9f; // Speed of the zombie
    private Animator anim;
    private GameObject button; // Button for evaulating when pressed in basement
    ...
    private void AnimationCheck()
    {
        ...
        else if (currAnimState == AnimationState.Run)
        {
            anim.SetBool("isAttacking", false);
    ...
    private void Wander()
    {
        ...
        currAnimState = AnimationState.Run;
        AnimationCheck();
    ...
    private void Chase()
    {
        ...
        if (canShout)
        {
            EventManager.TriggerEvent<ZombieScreamEvent, Vector3>(gameObject.transform.position);
            canShout = false;
        }
        currAnimState = AnimationState.Run;
        AnimationCheck();
